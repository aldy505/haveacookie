    ...
    }).addFields([
        {
            ...
            value:
                boardData.received.length === 0
                    ? noResultsMsg
                    : boardData.received.map((entry, index) => `${index + 1}. <@${entry.id}> - ${entry.received_count} ${emoji}`).join("\n"),
        },
        {
            name: "Given",
            value:
                boardData.given.length === 0 ? noResultsMsg : boardData.given.map((entry, index) => `${index + 1}. <@${entry.id}> - ${entry.given_count} ${emoji}`).join("\n"),
        },
    ]);
}

export function buildCookieCooldownEmbed(author: User, emoji: string, maxPerDay: number, sendAttemptSize: number | null = null): EmbedBuilder {
...
export function buildShopEmbed(prefix: string, emoji: string, prizes: Prize[]): EmbedBuilder {
    return new EmbedBuilder()
        .setColor(0x00008b)
        .setTitle("Prizes")
        .setDescription(prizes.map((prize, index) => `${index + 1}. **${prize.name}** (${prize.price} ${emoji})\n${prize.description}`).join("\n\n"))
