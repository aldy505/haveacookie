...
bot.on(Events.MessageCreate, async (msg: Message) => {
    ...
    if (msg.mentions.users.first() && msg.mentions.users.first()!.id !== msg.author.id && msg.content.includes(config.emoji)) {
        ...
        if (data.length + msg.mentions.users.size > config.maxPerDay) {
            const embed = buildCookieCooldownEmbed(msg.author, config.emoji, config.maxPerDay, msg.mentions.users.size);
    ...
    if (!alertChannel || alertChannel.type === ChannelType.GuildCategory || !alertChannel.isSendable()) {
        throw new Error("Configured prizePurchaseAlertChannel is not a text-based channel");
    }

    await alertChannel.send({ embeds: [buildPurchaseAlertEmbed(msg.author, chosenItem, config.emoji)] });
});
